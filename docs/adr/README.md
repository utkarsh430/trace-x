...
| [0053](0053-silver-canonical-events.md) | Silver: validated, exactly deduplicated, late-tagged canonical events, with quarantine and Bronze-to-Silver conservation. §7 amended by [0052](0052-bronze-ingest-topology.md) Amendment 1, point 6: after retention, a new checkpoint starts at the first live Bronze version. **Amendment 1:** the canonical and `late_events` MERGEs are bounded on `occurred_at`. **Amendment 2 (Step 13):** a micro-batch's cost is fixed, not proportional, so the sink asks Spark six fewer questions, re-derives `late_events` only when it can change it, and holds the canonical table to exactly one row per batch identity | 3 | Proposed (Amendments 1 and 2 Proposed) |
