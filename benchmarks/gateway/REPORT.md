...
> run recorded as `run_id: load-20260913-gateway-c86c6cdd`, which `make check-claims` resolves.
>
...
## Run — `run_id: load-20260913-gateway-c86c6cdd`
...
| commit | `c86c6cdd82da04dabaea64630d49e714a7171008` |
| dirty worktree | false |
| started / finished | 2026-09-13T01:30:19.459706Z / 2026-09-13T01:40:20.732494Z |
...
## Latency — `run_id: load-20260913-gateway-c86c6cdd`
...
| p90 | 3.53 ms | — |
| p95 | 6.92 ms | — |
| p99 | 25.30 ms | 0.71 ms |
| avg | 2.83 ms | — |
| min / max | 0.02 / 157.55 ms | — |

## Load actually offered — `run_id: load-20260913-gateway-c86c6cdd`

| quantity | value |
|---|---|
| requests | 299,999 |
| iterations started | 299,999 |
| iterations dropped | 0 |
| run duration | 600.02 s |
| achieved rate | 499.98 req/s |

## Responses — `run_id: load-20260913-gateway-c86c6cdd`
...
| 429 | 0 |
| 5xx | 0 |
| degraded decisions | 299,999 |
| … of which `history_incomplete` (store warming; expected on a cold run) | 299,999 |
| … degraded for any OTHER reason | 0 |
| feature store memory at end (`store=features`) | 548.1 MiB |
| feature store keys evicted | 0 |
| cache store memory at end (`store=cache`) | 128.0 MiB |
| cache store keys evicted (permitted; nothing in it decides) | 215,959 |

## Decision mix — `run_id: load-20260913-gateway-c86c6cdd`
...
## Exit conditions — `run_id: load-20260913-gateway-c86c6cdd`
...
|---|---|---|---|
| `requests_were_made` | INTEGRITY | PASS | 299999 iterations completed; a run with no data substantiates nothing |
...
| `no_unexpected_degradation` | TARGET | PASS | 0 decisions were degraded for a reason other than the store warming (unavailable store, refused write, unavailable limiter). 299999 carried `history_incomplete`, which a cold store reports by design until it has recorded for the widest declared lookback |
...
| `p99_under_budget` | TARGET | PASS | p99 25.30 ms against a 100 ms budget |
| `p50_under_budget` | TARGET | PASS | p50 1.73 ms against a 20 ms budget |

Displayed figures are rounded for reading; the unrounded values are in
`eval/manifest/load-20260913-gateway-c86c6cdd.json`.
