...
from trace_core.features import REFERENCE_FEATURES, FeatureRegistry, FeatureSpec, FeatureValue
...
def test_an_uncovered_feature_is_unavailable_not_zero() -> None:
    ...
    result = spec.evaluate(_tx(MINIMAL))
...
def test_reading_the_value_of_an_unavailable_feature_raises() -> None:
    ...
    result = REFERENCE_FEATURES.get("geo_precision").evaluate(_tx(MINIMAL))
...
def test_a_covered_feature_computes_normally() -> None:
    spec = REFERENCE_FEATURES.get("geo_precision")
    result = spec.evaluate(_tx(FULL, latitude=51.5, longitude=-0.1))
...
def test_truthiness_tracks_availability() -> None:
    ...
    assert bool(REFERENCE_FEATURES.get("amount_magnitude").evaluate(_tx(MINIMAL)))
    assert not bool(REFERENCE_FEATURES.get("geo_precision").evaluate(_tx(MINIMAL)))
...
def test_feature_values_are_immutable() -> None:
    result = REFERENCE_FEATURES.get("amount_magnitude").evaluate(_tx(MINIMAL))
...
def test_evaluate_all_reports_absence_rather_than_omitting_it() -> None:
    ...
    results = REFERENCE_FEATURES.evaluate_all(_tx(MINIMAL))
...
def test_a_partial_coverage_source_yields_unavailable_end_to_end() -> None:
    ...
    results = REFERENCE_FEATURES.evaluate_all(transaction)
    assert not results["geo_precision"].is_available
